...
export async function POST(request) {
    ...
    const [result, fields] = await conn.execute("SELECT userID, bookID FROM shopping_cart WHERE userID=?", [rowID])
    ...
    if(destruct.affectedRows <= 0) return Response.json({approved: false})
